// lib/screens/messages/group_resource.dart
...
class GroupResourceFolder {
  ...
  factory GroupResourceFolder.fromJson(Map<String, dynamic> json) {
    return GroupResourceFolder(
      id: json['id'],
      name: json['name'],
...
class GroupResourceFile {
  ...
  factory GroupResourceFile.fromJson(Map<String, dynamic> json) {
    return GroupResourceFile(
      id: json['id'],
      name: json['name'],
...
class GroupResourcePage extends StatefulWidget {
  ...
  @override
  State<GroupResourcePage> createState() => _GroupResourcePageState();
}

class _GroupResourcePageState extends State<GroupResourcePage> {
  List<Map<String, dynamic>> currentPath = [];
  List<GroupResourceFolder> folders = [];
  List<GroupResourceFile> files = [];
  bool isLoading = true;
  String? errorMessage;
  int? currentFolderId;
    ...
    try {
      ...
      setState(() {
        folders = (data['folders'] as List)
            .map((folder) => GroupResourceFolder.fromJson(folder))
            .toList();
        
        files = (data['files'] as List)
            .map((file) => GroupResourceFile.fromJson(file))
            .toList();
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                showSearch(
                  context: context,
                  delegate: ResourceSearchDelegate(files, widget.conversationId),
                );
              },
            ),
            PopupMenuButton(
                  ...
                  : errorMessage != null
                      ...
                      : ListView(
                          ...
                          children: [
                            ...
                            // Files
                            ...files.map((file) => ResourceFileTile(
                              file: file,
            ...
            child: Wrap(
              children: [
                ...
                ListTile(
                  ...
                  title: const Text('Post Reference to Chat'),
                  onTap: () {
                    Navigator.pop(context);
                    final ref = {
                      'type': 'file',
  ...
  void _copyFolderReference(BuildContext context, GroupResourceFolder folder) {
    // Navigate back to chat and add folder reference
  ...
  void _copyReference(BuildContext context, GroupResourceFile file) {
    // Navigate back to chat and add file reference
    Navigator.popUntil(context, (route) => route.settings.name == '/group_chat' || route.isFirst);
  ...
  void _deleteFolder(GroupResourceFolder folder) async {
  ...
  void _deleteFile(GroupResourceFile file) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
...
class ResourceSearchDelegate extends SearchDelegate {
  final List<GroupResourceFile> resources;
  final int conversationId;

  ResourceSearchDelegate(this.resources, this.conversationId);
